...
% Determine the mean by which to calibrate the  peaks and valleys
    ...
    mean_intensity                              =   mean(monitor_intensity_data,2);
    mean_main_intensity                         =   mean(main_intensity_data,2);
...
% compressed/stretched datapoints between a peak and the next valley.
    ...
    coupled_indices_intensities_peakvalley                          =   [index_peaks_mean,intensity_peak_mean;index_valleys_mean,intensity_valley_mean];
...
% Plot old and calibrated intensity data
    figure;
    plot(wavenumber_data,mean_intensity,'--r')
    hold on
    plot(wavenumber_data(sorted_coupled_indices_intensity_peakvalley_mean(:,1)),...
        sorted_coupled_indices_intensity_peakvalley_mean(:,2),'ks')
    plot(wavenumber_data,monitor_intensity_data(:,10))
    hold off
    xlabel('Wavenumber (cm^-^1)')
    ylabel('Intensity')
    legend('Mean of all monitor signals','Peaks and valleys of mean','Monitor signal')
    title('Monitor signals of gas and reference measurement')
    
    figure;
%     plot(wavenumber_data,mean_main_intensity,'--rs')
    hold on
    plot(wavenumber_data,main_intensity_data(:,6:15))
    hold off
    xlabel('Wavenumber (cm^-^1)')
    ylabel('Intensity')
    title('Main signals of gas and reference measurement')
    
    figure;
%     plot(wavenumber_data,mean_intensity,'--rs')
    hold on
    plot(wavenumber_data,adjusted_intensity_data_monitor(:,6:15))
    hold off
    xlabel('Wavenumber (cm^-^1)')
    ylabel('Intensity')
    title('Calibrated monitor signals of gas and reference measurement')

    figure;
%     plot(wavenumber_data,mean_main_intensity,'--rs')
    hold on
    plot(wavenumber_data,adjusted_intensity_data_main(:,6:15))
    hold off
    xlabel('Wavenumber (cm^-^1)')
    ylabel('Intensity')
    title('Calibrated main signals of gas and reference measurement')
end