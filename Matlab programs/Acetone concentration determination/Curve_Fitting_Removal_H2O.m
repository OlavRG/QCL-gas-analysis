...
function [ output1, output2 ] = Curve_Fitting_Removal_H2O(input1, input2)
...
if exist('WavenumberCalibration_H20.mat','file') == 2
    ...
    cal_wavenumber_all = cal_H2O.H2O_Calibration_peaks.wavenumber;
    cal_wavenumber = cal_wavenumber_all(2:end-1);
    cal_intensity_all = cal_H2O.H2O_Calibration_peaks.intensity;
    cal_intensity = cal_intensity_all(2:end-1);
else
    cal_wavenumber = 0;
end
...
% plot (data_wavenumber, data_absorbance,'k');
% hold on;

%% Fit Gaussian around H2O peaks


absorbance_remove_H2O = data_absorbance_region;


for k = 1:length(max_indexes)
    ind = [];
    ind = [max_indexes(k)-25 : max_indexes(k)+25];
    ...
    absorbance_remove_H2O (ind) =  absorbance_remove_H2O(ind) - y_fitting;
...
%     plot (data_wavenumber, absorbance_remove_H2O,'m');


%% Output

output1 = curve_fitting;
output2 = absorbance_remove_H2O;
