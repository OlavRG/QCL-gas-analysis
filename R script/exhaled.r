...
root <- file.path("~", "Dropbox", "Cardioformatics", "CTMM_management",
                  "projects", "Adonis_exhaled")
...
## import
if (use == "raw") {
    dat.dir <- file.path(root, "data", "Link_to_Final Data TUDelft", "raw")
    ...
    dat.dir <- file.path(root, "data", "Link_to_Final Data TUDelft",
                         "smoothed")
    file.nams <- dir(dat.dir)
    ## i <- 1 ## debugging
    files <- list()
...
##               paste(use, scal.meth, "healthy.vs.asthma.alldata.csv",
##                     sep=".")),
##           row.names = TRUE)
...
##           file = file.path(root, "results", "tables",
##               paste(use, scal.meth, "healthy.vs.CF.alldata.csv",
##                     sep=".")),
##           row.names = TRUE)
